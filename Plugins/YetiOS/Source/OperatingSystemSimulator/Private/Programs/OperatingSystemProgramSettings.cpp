...
UOperatingSystemProgramSettings* UOperatingSystemProgramSettings::LoadSettings(const TWeakObjectPtr<UOperatingSystemBaseProgram> OwningProgram)
{
	...
	const auto LoadSlotName = ProxyLoadInstance->Internal_GetSaveSlotName();
...
{
	if (Internal_CanSave() && bHasBlueprintPreSave)
	{
		K2_PreSave();
		return UGameplayStatics::SaveGameToSlot(this, SaveSlotName, UserIndex);
	}

	OS_LOG_ERROR(ProgramSettings, FString::Printf(TEXT("Failed to save %s (%s)."), *ParentProgram->GetProgramName(), *ParentProgram->GetIdentifier().ToString()));
	return false;
}
...
FString UOperatingSystemProgramSettings::Internal_GetSaveSlotName() const
{
	const auto OperatingSystem = ParentProgram->GetOperatingSystem();
	const auto ParentDevice = OperatingSystem->GetParentDevice();
