...
struct OPERATINGSYSTEMSIMULATOR_API FOperatingSystemUser
{
	...
	void Reset()
	{
		Username = Password = Email = FirstName = LastName = FText();
		Icon.Reset();
		OwnedProgramIdentifiers.Reset();
	}

	void UpdateDetails(const FOperatingSystemUser& NewDetails)
	{
#define OS_UPDATE_USER(Name) if (!NewDetails.Name.IsEmptyOrWhitespace()) \
		{ \
			Name = NewDetails.Name; \
		}

		OS_UPDATE_USER(Email)
		OS_UPDATE_USER(Password)
		OS_UPDATE_USER(Username)
		OS_UPDATE_USER(FirstName)
		OS_UPDATE_USER(LastName)
#undef OS_UPDATE_USER

		if (!NewDetails.Icon.IsNull())
		{
			Icon = NewDetails.Icon;
		}

		if (!NewDetails.OwnedProgramIdentifiers.IsEmpty())
		{
			OwnedProgramIdentifiers = NewDetails.OwnedProgramIdentifiers;
		}
	}
	...
	FORCEINLINE FOperatingSystemUser& operator=(const FOperatingSystemUser& Other)
	{
		if (this != &Other)
		{
			...
			Icon = Other.Icon;
			OwnedProgramIdentifiers = Other.OwnedProgramIdentifiers;
		}
	...
	{
		return Username.EqualToCaseIgnored(Other.Username) &&
			Password.EqualToCaseIgnored(Other.Password) &&
			Email.EqualToCaseIgnored(Other.Email) &&
			FirstName.EqualToCaseIgnored(Other.FirstName) &&
			LastName.EqualToCaseIgnored(Other.LastName);
	}
	...
	explicit FOperatingSystemUser(const FText& Username)
		: Username(Username)
		, Password(FText())
		, Email(FText())
		, FirstName(FText())
	...
	explicit FOperatingSystemUser(const FString& Username)
...
struct FOperatingSystemStoreReviewUser
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = OperatingSystemStoreReviewUser)
	FText Username;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = OperatingSystemStoreReviewUser)
	FText ReviewDescription;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = OperatingSystemStoreReviewUser, meta = (UIMin = 1, ClampMin = 1, UIMax = 5, ClampMax = 5))
	int32 SubmittedRating;

	FORCEINLINE bool IsValid() const { return !Username.IsEmptyOrWhitespace() && SubmittedRating >= 0; }

	FORCEINLINE bool operator==(const FOperatingSystemStoreReviewUser& Other) const
	{
		return Username.EqualToCaseIgnored(Other.Username) && ReviewDescription.EqualToCaseIgnored(Other.ReviewDescription) && SubmittedRating == Other.SubmittedRating;
	}

	FORCEINLINE friend uint32 GetTypeHash(const FOperatingSystemStoreReviewUser& Other)
	{
		return GetTypeHash(Other.Username.ToString()) ^ GetTypeHash(Other.ReviewDescription.ToString()) ^ GetTypeHash(Other.SubmittedRating);
	}

	FOperatingSystemStoreReviewUser()
		: Username(FText())
		, ReviewDescription(FText())
		, SubmittedRating(INDEX_NONE)
	{
	}
	...
	FOperatingSystemStoreReviewUser(const FOperatingSystemStoreReviewUser& Other)
		...
		, SubmittedRating(FMath::Clamp(Other.SubmittedRating, 1, 5))
	{
	}
};
...
struct FOperatingSystemStoreProgramData
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = OperatingSystemStoreProgramData)
	TSoftClassPtr<class UOperatingSystemBaseProgram> Program;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = OperatingSystemStoreProgramData)
	TSet<FOperatingSystemStoreReviewUser> ReviewUsers;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = OperatingSystemStoreProgramData, meta = (DisplayThumbnail = "true", AllowedClasses = "Texture,MaterialInterface"))
	TArray<TSoftObjectPtr<UObject>> Media;

	FORCEINLINE bool operator==(const FOperatingSystemStoreProgramData& Other) const
	{
		return Program == Other.Program && ReviewUsers.Array() == Other.ReviewUsers.Array() && Media == Other.Media;
	}

	FORCEINLINE friend uint32 GetTypeHash(const FOperatingSystemStoreProgramData& Other)
	{
		uint32 OtherHash = 0;
		for (const auto& It : Other.ReviewUsers)
		{
			OtherHash += GetTypeHash(It);
		}

		for (const auto& It : Other.Media)
		{
			OtherHash += GetTypeHash(It);
		}
		
		return GetTypeHash(Other.Program) ^ OtherHash;
	}

	FOperatingSystemStoreProgramData() = default;
	FOperatingSystemStoreProgramData(const TSoftClassPtr<class UOperatingSystemBaseProgram>& Program,
	...
	FOperatingSystemStoreProgramData(const TSoftClassPtr<class UOperatingSystemBaseProgram>& Program, const FOperatingSystemStoreReviewUser& NewUser)
		: Program(Program)
	{
		ReviewUsers.Emplace(NewUser);
	}
};

USTRUCT(BlueprintType)
struct FOperatingSystemStoreApps
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = OperatingSystemStoreApps)
