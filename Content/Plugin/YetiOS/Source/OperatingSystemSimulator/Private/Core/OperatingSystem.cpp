...
#define LOCTEXT_NAMESPACE "OperatingSystem"
...
void UOperatingSystem::Internal_LoadSettings(const TWeakObjectPtr<const UOperatingSystemSaveGame> LoadGameInstance, FGenericError& OutError)
{
	...
	const auto UserManager = UOperatingSystemStatics::GetUserManager(this);
	const auto& Users = UserManager->GetAllUsersForOS(this).Array();
	if (Users.IsValidIndex(0))
	{
		CurrentUser = Users[0];
	}
...
void UOperatingSystem::UninstallProgram(const FName& ProgramIdentifier, FGenericError& OutErrorIfAny)
{
	UClass* ProgramClass = nullptr;
	for (const auto& It : InstalledPrograms)
	{
		if (It->GetIdentifier() == ProgramIdentifier)
		{
			ProgramClass = It.GetClass();
			break;
		}
	}

	UninstallProgramByClass(ProgramClass, OutErrorIfAny);
}
	...
	FGenericError& OutErrorIfAny)
{
	ON_SCOPE_EXIT
	{
		if (OutErrorIfAny.IsValid())
		{
			AddNotification(OutErrorIfAny, EOperatingSystemNotificationCategory::OperatingSystem);
		}
	};

	OutErrorIfAny.Reset();
	const auto ProgramName = ProgramClass.LoadSynchronous()->GetDefaultObject<UOperatingSystemBaseProgram>()->GetProgramName();
	
	if (!ParentDevice->IsRunning())
	{
		OutErrorIfAny = MAKE_ERROR("ERR_PRG_UNINSTALL", FString::Printf(TEXT("Unable to uninstall %s while device is not in running state."), *ProgramName));
		return;
	}
	
	UOperatingSystemBaseProgram* FoundProgram = nullptr;
	for (const auto& It : InstalledPrograms)
	{
		if (It.GetClass() == ProgramClass.Get())
		{
			FoundProgram = It;
			break;
		}
	...
	if (!IsValid(FoundProgram))
	{
		OutErrorIfAny = MAKE_ERROR("ERR_PRG_UNINSTALL", FString::Printf(TEXT("%s is not installed."), *ProgramName));
		return;
	}

	if (FoundProgram->IsSystemInstalledProgram())
	{
		OutErrorIfAny = MAKE_ERROR("ERR_PRG_UNINSTALL", FString::Printf(TEXT("Cannot uninstall system program '%s'"), *ProgramName));
		return;
	}

	if (IsProgramRunning(FoundProgram))
	{
		OutErrorIfAny = MAKE_ERROR("ERR_PRG_UNINSTALL", FString::Printf(TEXT("%s is still running. Close the program before uninstalling."), *ProgramName));
		return;
	}

	const auto ProgramSpaceInMB = FoundProgram->GetSpaceRequiredInMB();
	const auto SpaceInBytes = UOperatingSystemStatics::ConvertMB_ToBytes(ProgramSpaceInMB);
	FoundProgram->GetParentPartition()->FreeupSpace(SpaceInBytes);
	
	InstalledPrograms.Remove(FoundProgram);

	OnProgramUninstalled.Broadcast(FoundProgram);
	UOperatingSystemSaveGame::SaveGame(ParentDevice.Get());

	FFormatNamedArguments Args;
	Args.Emplace(TEXT("Program"), FText::FromString(FoundProgram->GetProgramName()));

	FOperatingSystemNotification Notification(LOCTEXT("ProgramUninstalledNotifyTitle", "Program Uninstalled"),
		FText::Format(LOCTEXT("ProgramUninstalledNotifyDescription", "{Program} successfully removed."), Args),
		EOperatingSystemNotificationCategory::OperatingSystem);
	...
	FGenericError& OutErrorIfAny)
{
	...
	return InstallProgramFromClass(OutErrorIfAny, ProgramClassToInstall);
}
	...
	UOperatingSystemPartition* TargetPartition)
{
	...
	{
		OutErrorIfAny = MAKE_ERROR("ERR_NULL_PRG", "Unable to install null program.");
		AddNotification(OutErrorIfAny, EOperatingSystemNotificationCategory::OperatingSystem);
		return nullptr;
	}
	
	return Internal_InstallProgram(ProgramClass, IsValid(TargetPartition) ? TargetPartition : GetOperatingSystemPartition(), OutErrorIfAny);
}
...
{
	const auto UserManager = UOperatingSystemStatics::GetUserManager(this);
	if (UserManager->CreateNewUser(this, NewUser))
	{
		if (bSetAsActive)
		{
			CurrentUser = NewUser;
...
bool UOperatingSystem::LoginUser(const FText& Username, const FText& Password)
{
	...
	const auto UserManager = UOperatingSystemStatics::GetUserManager(this);
	if (UserManager->FindUserByUsername(Username, TargetUser))
	{
		TargetUser.Password = Password;
		UserManager->ForEachUserInOS(this, [this, &bSuccess, TargetUser](const FOperatingSystemUser& ItUser)
		{
			if (TargetUser == ItUser)
			{
				CurrentUser = ItUser;
				bSuccess = true;
				return true;
...
bool UOperatingSystem::AuthorizeUser(const FText& Email, const FText& Password, FOperatingSystemUser& OutAuthorizedUser) const
{
	const auto UserManager = UOperatingSystemStatics::GetUserManager(this);
	if (UserManager->FindUserByEmail(Email, OutAuthorizedUser))
	{
		return OutAuthorizedUser.Password.EqualToCaseIgnored(Password);
	}
...
{
	const auto UserManager = UOperatingSystemStatics::GetUserManager(this);
	return UserManager->UpdateUserDetails(CurrentUser, UpdatedDetails);
}
...
{
	return IsValid(GetInstalledProgram(TestClass));
}
...
{
	const auto UserManager = UOperatingSystemStatics::GetUserManager(this);
	return UserManager->GetAllUsersForOS(this).Array();
}
	...
	FGenericError& OutError)
{
	ON_SCOPE_EXIT
	{
		if (OutError.IsValid())
		{
			AddNotification(OutError, EOperatingSystemNotificationCategory::OperatingSystem);
		}
	};
	...
	FFormatNamedArguments Args;
	Args.Emplace(TEXT("Program"), FText::FromString(NewProgram->GetProgramName()));
	
	FOperatingSystemNotification Notification(LOCTEXT("ProgramInstalledNotifyTitle", "Program Installed"),
		FText::Format(LOCTEXT("ProgramInstalledNotifyDescription", "{Program} was successfully installed."), Args),
		EOperatingSystemNotificationCategory::OperatingSystem);
