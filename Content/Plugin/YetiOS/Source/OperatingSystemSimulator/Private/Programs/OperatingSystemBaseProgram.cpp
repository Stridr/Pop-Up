	...
	, ParentPartition(nullptr)
{
	...
	CTOR_LOAD_CLASS(UUserWidget, IconWidgetClass, "/OperatingSystemSimulator/OperatingSystemContent/Shared/Blueprints/Widgets/Icons/WBP_ProgramIcon");
	CTOR_LOAD_CLASS(UUserWidget, TaskbarButtonClass, "/OperatingSystemSimulator/OperatingSystemContent/Shared/Blueprints/Widgets/Common/WBP_TaskbarButton");

	CTOR_LOAD_OBJ(UMaterialInstanceConstant, IconObject, "/OperatingSystemSimulator/OperatingSystemContent/Shared/Materials/MI_Icon_DefaultApp");
	...
	const bool bInstalledWithOS /*= false*/)
{
	...
	ProxyProgram->DataObj = UOperatingSystemDataObject::CreateDataObj<UOperatingSystemProgramData>(ProxyProgram, ProxyProgram->DataClass);
	if (ProxyProgram->bRequiresData && !IsValid(ProxyProgram->DataObj))
	{
		OutError = MAKE_ERROR("ERR_DATA_NULL", FString::Printf(TEXT("Data object was not created for %s"), *ProxyProgram->GetClass()->GetName()));
		OwningOS->AddNotification(OutError, EOperatingSystemNotificationCategory::Application);
		ProxyProgram->MarkAsGarbage();
		ProxyProgram = nullptr;
		return nullptr;
...
{
	if (ProgramData.Program.IsNull())
	{
		return false;
	}

	const auto ProgramCDO = ProgramData.Program.LoadSynchronous()->GetDefaultObject<UOperatingSystemBaseProgram>();
	return ProgramCDO->StorePrice < 0.01;
}
...
{
	if (IsValid(CachedIconWidget))
	{
		return CachedIconWidget;
	}
	...
	CachedIconWidget = UOperatingSystemProgramIconWidget::CreateIconWidget(this);
	return CachedIconWidget;
}
...
void UOperatingSystemBaseProgram::Internal_Validate(FGenericError& OutError) const
{
	...
	if (bRequiresData && DataClass.IsNull())
	{
		OutError = MAKE_ERROR("ERR_PROGRAM", FString::Printf(TEXT("%s validation error: Data class required but specified null."), *GetClass()->GetName()));
		return;
	}
...
{
	const auto ParentDevice = ParentOS->GetParentDevice();
	const auto ParentDeviceActor = ParentDevice->GetParentDeviceActor();
	const auto DeviceID = ParentDeviceActor->GetDeviceActorUniqueID();
	return FString::Printf(TEXT("%s_%s"), *Identifier.ToString().ToLower(), *DeviceID);
}
