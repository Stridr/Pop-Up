...
UItemBase* UInventoryComponent::FindNextItemByID(UItemBase* ItemIn) const
{
	if (ItemIn)
	{
		...
		//we have an array and an access operator :: thats
		{
...
FItemAddResult UInventoryComponent::HandleNonStackableItem(UItemBase* InputItem)
{
	...
	if (FMath::IsNearlyZero(InputItem->GetItemSingleWeight()) || InputItem->GetItemSingleWeight() < 0)
	{
		return FItemAddResult::AddedNone(FText::Format(
			FText::FromString("Could not add {0} to the inventory . item has invalid weight "),
	...
	return FItemAddResult::AddedAll(1, FText::Format(
		                                FText::FromString("Successfully added a signle {0} to the inventory  "),
		                                InputItem->TextData.Name));
...
FItemAddResult UInventoryComponent::HandleAddItem(UItemBase* InputItem) //Entry Point
{
	...
	return FItemAddResult::AddedNone(
		FText::FromString("Try add item fallthrough error . get Owner() check somehow failed"));
}
